...
<c:if test="${empty Userlogin}">
      <form name="" action="" id="usersForm">
      	<button type="submit" class="usersInfo" id="hSignUp">회원가입</button>&nbsp;
      	<button type="submit" class="usersInfo" id="hLogin">로그인</button>
      	<button type="submit" class="usersInfo" id="" style="width: 80px">id/pw 찾기</button>
      </form>  
</c:if>
<c:if test="${!empty Userlogin}">
 	안녕하세요. ${Userlogin.username}님!<br>
 	<form name="" action="" id="usersForm">
 		<button type="submit" class="usersInfo" id="hLogout">로그아웃</button>
 		<button type="submit" class="usersInfo" id="userUpdate" style="width: 80px">회원정보변경</button>
 	</form>
</c:if>
      <!-- 아이콘
       Facebook SVG
...
<script type="text/javascript">//회원관련 버튼 스크립트
	$(document).ready(function()
	{
		$("#hSignUp").on("click",function()
		{
			$("#usersForm").attr("action","signUpUI");
		});
		$("#hLogin").on("click",function()
		{
			$("#usersForm").attr("action","loginUI");
		});
		$("#hLogout").on("click",function()
		{
			$("#usersForm").attr("action","logout");
		});
		$("#userUpdate").on("click",function()
		{
			$("#usersForm").attr("action","userUpdate");
		});
