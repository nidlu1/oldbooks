...
<form id="boardForm">
	...
	<input type="text" placeholder="${bs} 검색어로 검색한 결과입니다" name="schWord">
...
<div>
	<table class="boardTable">
		...
		<tbody>
			<c:forEach items="${boardSchList}" var="b">
		...
		<c:if test="${empty boardSchList}">
			<h1>검색결과가 없습니다.</h1>
		</c:if>
	<div style="margin: 20px; font-size: 30px">	
		<c:if test="${prev}">
			<a href="schBoardUI/${startPage-1}">[&lt;]</a>
		</c:if>

		<c:forEach begin="${startPage}" end="${endPage}" var="index">
			<a href="schBoardUI/${index}">[${index}]</a>
		</c:forEach>
		
		<c:if test="${next}">
				<a href="schBoardUI/${endPage+1}">[&gt;]</a>	
		</c:if>
	</div>
</div>
